...
#### `$.raw`
Temporary disables default `shq` escaping for a single command invocation.
Please take in mind that might be potentially unsafe.
```ts
  let ch = 'ch'
  let cmd = `e${ch}o foo`
  (await $.raw`${cmd}`).stdout.trim() // foo
```
